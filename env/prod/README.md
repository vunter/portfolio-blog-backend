...
- `doppler` CLI instalado, autenticado, e service token salvo em `~/.doppler_token` (lido via `source ~/.doppler_token` pelos scripts — arquivo no formato `export DOPPLER_TOKEN=<service-token>`)
