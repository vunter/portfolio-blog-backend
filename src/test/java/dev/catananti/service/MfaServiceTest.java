...
class MfaServiceTest {
    ...
    // ============ TOTP algorithm compatibility (java-otp 0.4.0 -> 1.0.0) ============

    /**
     * Guards the ONE thing that must never change across a java-otp upgrade: the
     * codes an already-enrolled authenticator app produces must keep verifying.
     * A silent algorithm change here locks the owner out of their own account,
     * and no amount of "it compiles" would catch it.
     * <p>
     * The strategy is to not trust either library version. {@link #referenceTotp}
     * below is an independent RFC 6238 implementation written directly against
     * javax.crypto.Mac — the same construction java-otp 0.4.0 used (HMAC, dynamic
     * truncation on the low nibble of the last byte, 31-bit mask, modulo
     * 10^digits). It is first pinned to the published RFC 6238 Appendix B test
     * vectors, which are authoritative and independent of any library, and then
     * used to feed codes into the real MfaService verification path.
     * <p>
     * So: RFC 6238 == reference impl == what MfaService accepts.
     */
    @Nested
    @DisplayName("TOTP algorithm compatibility")
    class TotpAlgorithmCompatibilityTests {

        @Test
        @DisplayName("Reference implementation matches the published RFC 6238 Appendix B vectors")
        void referenceImplementation_MatchesRfc6238TestVectors() throws Exception {
            // RFC 6238 Appendix B: seed is the ASCII string "12345678901234567890",
            // T0 = 0, X = 30s, 8 digits, HMAC-SHA1.
            byte[] seed = "12345678901234567890".getBytes(StandardCharsets.US_ASCII);

            assertThat(referenceTotp(seed, 30, 59L, 8)).isEqualTo(94287082);
            assertThat(referenceTotp(seed, 30, 1111111109L, 8)).isEqualTo(7081804);
            assertThat(referenceTotp(seed, 30, 1111111111L, 8)).isEqualTo(14050471);
            assertThat(referenceTotp(seed, 30, 1234567890L, 8)).isEqualTo(89005924);
            assertThat(referenceTotp(seed, 30, 2000000000L, 8)).isEqualTo(69279037);
            assertThat(referenceTotp(seed, 30, 20000000000L, 8)).isEqualTo(65353130);
        }

        @Test
        @DisplayName("MfaService accepts a code generated by the pre-upgrade algorithm")
        void verifyTotp_AcceptsCodeFromReferenceAlgorithm() throws Exception {
            // A 160-bit secret of the shape MfaService.setupTotp() generates.
            ...
            // Same secret, same time step (30s) and same digit count (6) the
            // service is configured with in setUp() — i.e. the exact parameters
            // an already-enrolled authenticator is using.
            int code = referenceTotp(secretBytes, 30, Instant.now().getEpochSecond(), 6);
            ...
            when(redisTemplate.expire(anyString(), any(Duration.class))).thenReturn(Mono.just(true));
            when(mfaConfigRepository.findByUserIdAndMethod(USER_ID, "TOTP")).thenReturn(Mono.just(verifiedConfig));
            when(aesEncryptor.decrypt(ENCRYPTED_SECRET)).thenReturn(base64Secret);
            when(valueOperations.setIfAbsent(anyString(), eq("1"), any(Duration.class)))
                    .thenReturn(Mono.just(true));
            when(redisTemplate.delete(anyString())).thenReturn(Mono.just(1L));
            ...
            StepVerifier.create(mfaService.verifyTotp(USER_ID, totpCode))
                    .assertNext(result -> assertThat(result)
                            .as("a code from the pre-upgrade TOTP algorithm must still verify")
                            .isTrue())
                    .verifyComplete();
        }

        @Test
        @DisplayName("MfaService rejects a code from a DIFFERENT secret (guards against a vacuous pass)")
        void verifyTotp_RejectsCodeFromDifferentSecret() throws Exception {
            byte[] enrolledSecret = "12345678901234567890".getBytes(StandardCharsets.US_ASCII);
            byte[] attackerSecret = "09876543210987654321".getBytes(StandardCharsets.US_ASCII);

            int code = referenceTotp(attackerSecret, 30, Instant.now().getEpochSecond(), 6);
            String totpCode = String.format("%06d", code);
            ...
            when(valueOperations.increment(anyString())).thenReturn(Mono.just(2L));
            when(mfaConfigRepository.findByUserIdAndMethod(USER_ID, "TOTP")).thenReturn(Mono.just(verifiedConfig));
            when(aesEncryptor.decrypt(ENCRYPTED_SECRET))
                    .thenReturn(Base64.getEncoder().encodeToString(enrolledSecret));
        ...
        /**
         * RFC 6238 / RFC 4226 TOTP, implemented from the spec with no java-otp
         * involvement, so it cannot drift when java-otp does.
         */
        private int referenceTotp(byte[] secret, long timeStepSeconds, long epochSeconds, int digits)
                throws Exception {
            long counter = epochSeconds / timeStepSeconds;

            Mac mac = Mac.getInstance("HmacSHA1");
            mac.init(new SecretKeySpec(secret, "HmacSHA1"));
            byte[] hash = mac.doFinal(ByteBuffer.allocate(8).putLong(0, counter).array());

            // RFC 4226 section 5.4 dynamic truncation
            int offset = hash[hash.length - 1] & 0x0F;
            int binary = ByteBuffer.wrap(hash).getInt(offset) & 0x7FFF_FFFF;
